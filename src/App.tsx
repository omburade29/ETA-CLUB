...
const Select = ({ label, icon: Icon, options, ...props }: any) => (
  <div className="space-y-1.5 text-left">
    <label className="text-sm font-medium text-slate-700 flex items-center gap-2 uppercase tracking-wider text-xs font-bold">
      {Icon && <Icon size={16} className="text-slate-400" />}
      {label}
    </label>
    <select
      {...props}
      className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none appearance-none cursor-pointer"
...
export default function App() {
  ...
  const exportToExcel = () => {
    const formattedPayments = payments.map(p => ({
      Date: new Date(p.created_at + ' UTC').toLocaleString('en-IN', { 
    ...
    const groupedPayments = formattedPayments.reduce((acc, payment) => {
      const year = payment.Year || 'Unknown Year';
      if (!acc[year]) acc[year] = [];
      acc[year].push(payment);
      return acc;
    }, {} as Record<string, typeof formattedPayments>);

    // Create a sheet for each year
    const years = Object.keys(groupedPayments);
    if (years.length === 0) {
      const worksheet = XLSX.utils.json_to_sheet([]);
      XLSX.utils.book_append_sheet(workbook, worksheet, "Payments");
    } else {
      years.forEach(year => {
        const worksheet = XLSX.utils.json_to_sheet(groupedPayments[year]);
        const safeSheetName = year.replace(/[:\\/?*\[\]]/g, '').substring(0, 31);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="print:hidden flex-1 flex flex-col">
        ...
        >
          ...
          <h1 className="text-xl font-display font-bold text-slate-900">ETA COLLECTION</h1>
        </div>
