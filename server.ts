...
async function startServer() {
  ...
  app.post("/api/initiate-manual-payment", (req, res) => {
    try {
      ...
      const upiId = upiIdRow?.value || process.env.ADMIN_UPI_ID || "";
      const adminName = adminNameRow?.value || process.env.ADMIN_NAME || "";
  ...
  app.post("/api/admin/login", (req, res) => {
    ...
    const adminUser = process.env.ADMIN_USERNAME || "ETA Club";
    const adminPass = process.env.ADMIN_PASSWORD || "ENTC";
